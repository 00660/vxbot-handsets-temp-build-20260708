...
cv::Mat bitmap_to_cv_mat(JNIEnv *env, jobject bitmap) {
  ...
  if (AndroidBitmap_lockPixels(env, bitmap, (void **)&srcData) != ANDROID_BITMAP_RESULT_SUCCESS) {
  ...
  const size_t row_size = static_cast<size_t>(info.width) * 4;
  for (uint32_t row = 0; row < info.height; ++row) {
    memcpy(mat.ptr(row), srcData + static_cast<size_t>(row) * info.stride, row_size);
  }
  AndroidBitmap_unlockPixels(env, bitmap);
