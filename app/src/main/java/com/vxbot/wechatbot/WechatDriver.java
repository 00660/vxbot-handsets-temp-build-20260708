...
public final class WechatDriver {
    ...
    private boolean tapSearchEntryForBroadcast(Context context, BotConfig config) throws Exception {
        ...
        if (searchIcon == null) {
            ...
            int x = Math.round(width * 0.86f);
            int y = Math.round(height * 0.070f);
            hs.tap(x, y);
            BotLog.w(context, "broadcast.search.entry.resolution_fallback",
                    "截图未识别到放大镜，按当前分辨率比例点击微信右上角搜索 x=" + x + " y=" + y
                            + " screen=" + width + "x" + height
                            + " ocrCount=" + (screen == null ? 0 : screen.items.size()));
            broadcastStepDelay(config, "search_entry_resolution_fallback");
            return true;
        }
    ...
    private static String normalizeOcrContent(String value) {
        return NameNormalizer.contentKey(value);
    }

    private static boolean isLikelyOcrName(String text, String name) {
        return NameNormalizer.looseNameMatch(text, name);
    }
    ...
    private static String titleFromOcr(OcrHelper.Screen screen) {
        ...
        for (OcrHelper.OcrItem item : screen.items) {
            ...
            if (value.isEmpty()
                    ...
                    || "我".equals(value)
                    || value.matches("^\\d{1,2}:\\d{2}$")
                    || value.matches("^(P[0-9A-Za-z]*|dX.*|dY.*|Xv.*|Yv.*|Prs.*|Size.*)$")
                    || value.matches("^\\d+$")) {
                continue;
            }
